...
1. **强职责切分：** 将查询（`QueryService`）与事务（`Service`）分离，这是一种非常好的优化，能够有效避免长事务造成的数据库连接池锁定。
...
为了让任何高级开发者 产出更精准的方案，你的输入（Prompt）需要具备以下**架构要素**：

1. **Context (上下文)**：该模块在整个系统中的位置，其上下游是谁？
2. **Constraint (约束)**：并发量级（QPS）、延迟容忍度、数据一致性要求（强一致 vs 最终一致）。
3. **Boundary (边界)**：该模块**不负责**什么？明确界定职责范围。
4. **Contract (契约)**：输入的数据结构是什么？期望的输出和异常处理是什么？