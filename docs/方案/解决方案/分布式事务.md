...
TCC是一种基于业务层面的分布式事务解决方案。TCC将事务分为三个阶段：
...
Saga模式，是一种基于事件驱动的分布式事务解决方案。Saga将长事务分解为一系列短事务，每个短事务都有对应的补偿操作。
...
AT模式是阿里Seata框架提出的一种分布式事务解决方案，它通过自动生成反向SQL来实现事务回滚，对业务代码无侵入。
